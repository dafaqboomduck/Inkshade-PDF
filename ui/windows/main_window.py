...
from PyQt5.QtGui import QIcon, QPixmap, QPainter, QColor, QKeySequence, QIntValidator
from PyQt5.QtWidgets import (
    QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QFileDialog, QScrollArea, QLineEdit, QFrame,
    QMessageBox, QSpacerItem, QSizePolicy, QApplication,
    QToolButton, QProgressDialog, QLabel
)
...
class MainWindow(QMainWindow):
    ...
    def _init_controllers(self):
        ...
        # PDF viewer (manages page display)
    ...
    def _create_toolbar(self):
        ...
        # File operations
        self._add_toolbar_button("open-icon.png", "Open PDF (Ctrl+O)", 
                                self.open_pdf)
        self._add_toolbar_button("close-icon.png", "Close PDF (Ctrl+W)", 
                                self.close_pdf)
        ...
        self._add_toolbar_button("search-icon.png", "Search (Ctrl+F)", 
        ...
        # Tools
        self._add_toolbar_button("annotate-icon.png", "Annotate Selection", 
                                self.show_annotation_toolbar)
        self._add_toolbar_button("draw-icon.png", "Draw", 
                                self.show_drawing_toolbar)
        self._add_toolbar_button("save-icon.png", "Save PDF (Ctrl+S)", 
                                self.save_annotations_to_pdf)
    ...
    def _add_toolbar_button(self, icon_name: str, tooltip: str, 
                           callback) -> QToolButton:
        """Add a button to the toolbar."""
        btn = self.create_icon_button(f"resources/icons/{icon_name}", 
                                     tooltip, self.top_frame)
    ...
    def _create_zoom_controls(self):
        ...
        self._add_toolbar_button("zoom-out-icon.png", "Zoom Out", 
                                lambda: self.adjust_zoom(-20))
        ...
        self._add_toolbar_button("zoom-in-icon.png", "Zoom In", 
                                lambda: self.adjust_zoom(20))
    ...
    def _create_floating_toolbars(self):
        ...
        self.drawing_toolbar.drawing_mode_changed.connect(
            self._on_drawing_mode_changed
        )
        self.drawing_toolbar.tool_changed.connect(
            self._on_drawing_tool_changed
        )
    ...
    def _apply_theme(self):
        ...
        for toolbar in [self.search_bar, self.annotation_toolbar, 
                       self.drawing_toolbar]:
    ...
    def create_icon_button(self, icon_path: str, tooltip: str, 
                          parent: QWidget = None) -> QToolButton:
    ...
    def _handle_zoom_change(self, new_zoom_percent: int):
        ...
        try:
            ...
            if self.pdf_reader.doc:
                ...
                # Restore position
                QTimer.singleShot(10, lambda: self._restore_scroll_position(
    ...
    def toggle_theme(self):
        ...
        if self.pdf_reader.doc:
            ...
            # Restore position
            QTimer.singleShot(0, lambda: self._restore_and_repaint(
                current_page_index, offset_in_page))
    ...
    def _create_annotation_from_selection(self, annotation_type: AnnotationType, 
                                         color: tuple):
    ...
    def _on_drawing_tool_changed(self, tool: AnnotationType, color: tuple, 
                                 stroke_width: float, filled: bool):
    ...
    def save_annotations_to_pdf(self) -> bool:
        ...
        saving_to_same_file = (os.path.abspath(output_path) == 
                               os.path.abspath(original_pdf_path))
        
        # Close document before saving
        self.pdf_reader.close_document()
        
        # Create progress dialog
        progress = QProgressDialog("Preparing to export annotations...", 
                                  None, 0, 100, self)
    ...
    def copy_selected_text(self):
        ...
        if self.pdf_reader.doc is None or self.current_page_index not in self.loaded_pages:
            QMessageBox.warning(self, "No Page Loaded", 
                               "Please load a PDF document first.")
            return
        ...
        if selected_text:
            ...
            QMessageBox.information(self, "No Selection", 
                                   "No text has been selected on the current page.")
